...
## load in lobbyview
lobby_client <- read.csv("01_raw/lobbyview_20250103/clients_codebook/clients.csv", stringsAsFactors = F)
...
nrow(clientnm_issues)
# NOTE: None left anymore!
...
# report <- read.csv("01_raw/lobbyview_20250103/reports_codebook/reports.csv", stringsAsFactors = F)

# glimpse(report)

# report <- report[which(report$lob_id %in% clientnm_issues$lob_id), ]
...
sum(duplicated(d$lob_id) & !is.na(d$lob_id))
# NONE
...
exposure_orbis_lobby_wide$lob_id[is.na(exposure_orbis_lobby_wide$lob_id)] <- (-1) # for convenience. Ensures no merges screwed up b/c no client_uuids =(-1)
...
check <- (duplicated(exposure_orbis_lobby_wide$lob_id) & exposure_orbis_lobby_wide$lob_id!=(-1)) | (duplicated(exposure_orbis_lobby_wide$lob_id, fromLast=T) & exposure_orbis_lobby_wide$lob_id!=(-1))
# table check (is a vector of boolean values)
table(check)
# None true!
# View(exposure_orbis_lobby_wide[check, c("bvdid", "isin", "cc_expo_ew_2016_1", "gvkey", "hqcountrycode", "conm", "total_assets_usd_2016", "lob_id", "client_name")])
sum(duplicated(exposure_orbis_lobby_wide$lob_id) & exposure_orbis_lobby_wide$lob_id!=(-1))
# none

#which(d$exposure_orbis_lobby_wide=="054f36eb-b834-58b6-bf55-a0c89d6660eb")
out <- exposure_orbis_lobby_wide[check, c("bvdid", "isin", "cc_expo_ew_2016_1", "gvkey", "hqcountrycode", "conm", "total_assets_usd_2016", "lob_id", "client_name")]
...
# fix_duplicate_isins <- aggregate(duplicate_isins$lob_id, by=list(duplicate_isins$isin), FUN=function(x) paste(x, collapse="|"))
# ## collapse and separate client_uuids by vertical bar
# names(fix_duplicate_isins) <- c("isin", "lob_id")
...
#timespan <- 1999:2023
timespan <- apply(expand.grid(c(2001:2023), c(1:4)), 1, paste, collapse="_")
...
write.csv(exposure_orbis_lobby_wide, "02_processed/exposure_orbis_client_quarter_wide_REVISE_NEW.csv", row.names = F)
write.csv(exposure_orbis_lobby_long, "02_processed/exposure_orbis_client_quarter_long_REVISE_NEW.csv", row.names = F)
