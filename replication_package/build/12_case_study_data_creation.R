...
# Load data
df_new <- arrow::read_parquet("data/03_final/lobbying_df_quarterly_REVISE_normal_NEW.parquet")
...
df_kw |>
  ...
  select(isin, year, qtr, issue_text) |>
  head()

# Merge
df <- df_new |>
  left_join_check_obs(df_kw %>% select(isin, year, qtr, issue_text), by = c("isin", "year", "qtr"))
...
# 
# 
# exposure_orbis_lobbyview_long <- fread("data/02_processed/lobbying_df_quarterly_only_CLI_text.csv")
# 
