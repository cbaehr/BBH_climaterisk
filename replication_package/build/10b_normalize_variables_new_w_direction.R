...
#Load data
df <- arrow::read_parquet("data/03_final/lobbying_df_quarterly_REVISE_NEW_altclimatebills.parquet")
df <- data.frame(df)
...
arrow::write_parquet(df, "data/03_final/lobbying_df_quarterly_REVISE_normal_NEW_altclimatebills.parquet")

# # write csv
# fwrite(df, file="data/03_final/lobbying_df_quarterly_REVISE_normal_NEW_altclimatebills.csv")
# 
# names(df)
# 
...
# 
# 
# write_rds(df, "data/03_final/lobbying_df_quarterly_REVISE_normal_NEW_altclimatebills.rds")
# 
...
# 
# #Load data
# df <- arrow::read_parquet("data/03_final/lobbying_df_annual_REVISE_NEW_altclimatebills.parquet")
# 
# # df <- read_rds("data/03_final/lobbying_df_annual_REVISE_NEW_altclimatebills.rds")
# df <- data.frame(df)
# #Normalize variables for interpretation 
# 
# ## continuous variables in regression models
...
# table(df$year)
# 
# arrow::write_parquet(df, "data/03_final/lobbying_df_annual_REVISE_normal_NEW_altclimatebills.parquet")

# write rds
# write_rds(df, file="data/03_final/lobbying_df_annual_REVISE_normal_altclimatebills.rds")
# 
# # write csv
# fwrite(df, file="data/03_final/lobbying_df_annual_REVISE_normal_altclimatebills.csv", row.names = FALSE)
# 
# df_dta <- df[ , !names(df) %in% c("isin_all", "gov_entity", "issue_code", "issue_text",
...
#                                   "nace_secondary", "nace_main_section", "sic_secondary", "sic_primary",
#                                   "bvdsector", "primary_naics", "amount_num")]
# write_dta(df_dta, path="data/03_final/lobbying_df_annual_REVISE_normal_stata_altclimatebills.dta")
