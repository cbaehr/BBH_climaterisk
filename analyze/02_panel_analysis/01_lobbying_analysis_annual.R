...
# load data
df <- fread("data/03_final/lobbying_df_annual_REVISE_normal.csv")
...
df$CLI_annual <- as.numeric(df$CLI_annual)
...
models <- list(
  "(1)" = feglm(CLI_annual ~ cc_expo_ew, family = "binomial", df),
  "(2)" = feglm(CLI_annual ~ cc_expo_ew | year, family = "binomial", df),
  "(3)" = feglm(CLI_annual ~ cc_expo_ew + ebit + I(ebit/at) | year, family = "binomial", df),
  "(4)" = feglm(CLI_annual ~ cc_expo_ew + ebit + I(ebit/at) + us_dummy + total_lobby_annual | year, family = "binomial", df),
  "(5)" = feglm(CLI_annual ~ cc_expo_ew + ebit + I(ebit/at) + us_dummy + total_lobby_annual | year + gvkey, family = "binomial", df)
)
modelsummary(
  models
  ,stars = c('*' = .1, '**' = .05, '***' = .01)
