...
# Create sequential quarter numbers and filter for utilities
df <- df %>%
  mutate(t = as.numeric(factor(yearqtr, levels = sort(unique(yearqtr))))) %>%
  filter(industry == "Utilities")  # Filter for utilities industry only
...
shocks <- list(
  opportunity = list(
    shock_quarter = "33",  # ARRA: February 2009
    treatment_var = "op_expo_ew",
    name = "ARRA Shock"
  ),
  regulatory = list(
    shock_quarter = "59",  # Clean Power Plan: August 2015
    treatment_var = "rg_expo_ew",
    name = "Clean Power Plan"
  ),
  physical = list(
    shock_quarter = "67",  # CA Wildfires: Major outbreak in Q3 2017
...
write_rds(att_results, "results/dids/did_analysis_att_results_utilities.rds")
write_rds(event_study_results, "results/dids/did_analysis_event_study_results_utilities.rds")
...
create_event_study_plot <- function(results_subset, group_info) {
  ...
  treatment_type <- case_when(
    str_detect(group_info$shock_type, "ARRA") ~ "opportunity",
    str_detect(group_info$shock_type, "Clean Power Plan") ~ "regulatory",
    str_detect(group_info$shock_type, "CA Wildfires") ~ "physical",
    TRUE ~ "unknown"
  )
...
ggsave("results/Figures/did/utilities_att_results_combined.pdf", 
