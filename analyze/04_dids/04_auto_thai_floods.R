...
thailand_auto_companies <- c(
    ...
    "SUZUKI MOTOR CORPORATION", "DENSO CORPORATION", 
    "BOSCH LIMITED"
)

# are these companies in the df?
df_auto %>%
  filter(conm %in% thailand_auto_companies) %>%
  distinct(conm, industry)

# any of them not in the df?
setdiff(thailand_auto_companies, df_auto$conm)
...
df_auto %>%
  mutate(
    ...
    affected = ifelse(treated_thailand == 1, "Affected", "Not Affected")
  ) %>%
  ggplot(aes(x = date, y = as.numeric(log_CLI_amount), color = affected)) +
    ...
    theme_hanno() +
    labs(
      title = "Climate Lobbying Intensity of Auto Companies During Thai Floods",
      x = "Quarter",
      y = "Log Climate Lobbying Amount",
      color = "Thai Floods Impact"
    ) +
    theme(
...
ggsave("results/figures/did/auto_thai_floods_cli_intensity_time_series.pdf", width = 40, height = 20)
...
df_auto %>%
  ...
  ggplot(aes(x = date, y = as.numeric(CLI_quarter), color = factor(treated_thailand))) +
    geom_line(aes(group = factor(isin))) +
    geom_point(aes(shape = factor(treated_thailand))) +
    annotate("rect", 
             xmin = as.Date("2011-07-25"), 
             xmax = as.Date("2012-01-16"),
             ymin = -Inf, 
    ...
    theme_hanno() +
    labs(
      title = "Climate Lobbying Occurrence of Auto Companies During Thai Floods",
      x = "Quarter",
      y = "Climate Lobbying Occurrence",
      color = "Affected by Thai Floods"
    ) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
      legend.position = "bottom"
...
ggsave("results/figures/did/auto_thai_floods_cli_occurrence_time_series.pdf", width = 40, height = 20)
