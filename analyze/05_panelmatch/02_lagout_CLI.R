...
# load data
df <- read_rds("data/lobbying_df_quarterly_REVISE_normal_NEW.rds")
...
rm(list=setdiff(ls(), c("df_pm", "treatments", "outcomes", "covariates", 
...
results_files <- list.files("figures/", pattern = "pm_results_lagout.*\\.csv$", full.names = TRUE)

# Read and combine files using base R
results_df <- do.call(rbind, lapply(results_files, read.csv))

# Write final combined results
write.csv(results_df, file = paste0("figures/", "pm_results_lagout.csv"), row.names = FALSE)
