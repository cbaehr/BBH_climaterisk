...
## opportunity
# quarterly
table(df_c$op_expo_ew_yq_disc_deciles)
table(df_c$op_expo_ew_yqindustry_above_90)
table(df_c$op_expo_ew_yqindustry_above_95)
table(df_c$op_expo_ew_yqindustry_above_99)
table(df_c$op_expo_ew_yindustry_above_mean)

# yearly
table(df_c$op_expo_ew_y_disc_deciles)
table(df_c$op_expo_ew_yindustry_above_90)
table(df_c$op_expo_ew_yindustry_above_95)
table(df_c$op_expo_ew_yindustry_above_99)

## regulatory
# quarterly
table(df_c$rg_expo_ew_yq_disc_deciles)
table(df_c$rg_expo_ew_yqindustry_above_90)
table(df_c$rg_expo_ew_yqindustry_above_95)
table(df_c$rg_expo_ew_yqindustry_above_99)
table(df_c$rg_expo_ew_yindustry_above_mean)

# yearly
table(df_c$rg_expo_ew_y_disc_deciles)
table(df_c$rg_expo_ew_yindustry_above_90)
table(df_c$rg_expo_ew_yindustry_above_95)
table(df_c$rg_expo_ew_yindustry_above_99)

## physical
# quarterly
table(df_c$ph_expo_ew_yq_disc_deciles)
table(df_c$ph_expo_ew_yqindustry_above_90)
table(df_c$ph_expo_ew_yqindustry_above_95)
table(df_c$ph_expo_ew_yqindustry_above_99)
table(df_c$ph_expo_ew_yindustry_above_mean)

# yearly
table(df_c$ph_expo_ew_y_disc_deciles)
table(df_c$ph_expo_ew_yindustry_above_90)
table(df_c$ph_expo_ew_yindustry_above_95)
table(df_c$ph_expo_ew_yindustry_above_99)
...
df_c |>
  ...
  bind_rows(
    df_c |> group_by(yearqtr, rg_expo_ew_yqindustry_above_mean) |>
      summarise(money = sum(CLI_amount_quarter, na.rm = TRUE) / 10^6) |>
      rename(quantile = rg_expo_ew_yqindustry_above_mean) |> mutate(measure = "Regulatory"),
    df_c |> group_by(yearqtr, ph_expo_ew_yqindustry_above_mean) |>
      summarise(money = sum(CLI_amount_quarter, na.rm = TRUE) / 10^6) |>
...
plot_df <- df_c |>
  group_by(yearqtr, op_expo_ew_y_disc) |>
  summarise(money = sum(CLI_amount_quarter, na.rm = TRUE)  / 10^6) |>
  rename(quantile = op_expo_ew_y_disc) |> mutate(measure = "Opportunity") |>
  bind_rows(
    df_c |> group_by(yearqtr, rg_expo_ew_y_disc) |>
      summarise(money = sum(CLI_amount_quarter, na.rm = TRUE) / 10^6) |>
      rename(quantile = rg_expo_ew_y_disc) |> mutate(measure = "Regulatory"),
    df_c |> group_by(yearqtr, ph_expo_ew_y_disc) |>
      summarise(money = sum(CLI_amount_quarter, na.rm = TRUE) / 10^6) |>
      rename(quantile = ph_expo_ew_y_disc) |> mutate(measure = "Physical")
  ) |>
  ungroup() |>
  filter(!is.na(quantile)) |>
  # Transform quartiles into percentage ranges
