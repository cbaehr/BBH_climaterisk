...
models <- list(
  ...
  "(7)" = feols(log(CLI_dollars +1) ~ op_expo_ew_y + rg_expo_ew_y + ph_expo_ew_y + ebit + I(ebit/at) + us_dummy + total_lobby | year + gvkey, df)
)

#[which(df$conm!="BANK OF AMERICA CORPORATION"),]
library(censReg)
...
df.pre_imp <- df[, c("CLI", "op_expo_ew_y", "rg_expo_ew_y", "ph_expo_ew_y", "ebit", "at", "us_dummy", "total_lobby", "year", "report_quarter_code", "gvkey")]

varying <- c("CLI", "op_expo_ew_y", "rg_expo_ew_y", "ph_expo_ew_y", "ebit", "at", "total_lobby")
df.pre_imp_wide <- pivot_wider(df.pre_imp, names_from = c(year, report_quarter_code), values_from = all_of(varying))

### SETUP

init <- mice(df.pre_imp_wide, maxit = 0)
meth <- init$method
pred <- init$predictorMatrix
visSeq <- init$visitSequence

# avoid collinearity issues by letting only variables measured
# at the same point in time predict each other

## identify the year and quarter from all rows
nm <- regmatches(rownames(pred)[-c(1:2)], regexpr("[0-9]{4}_[0-9]{1}$", rownames(pred)[-c(1:2)]))
## check each rowname on each column name for equivalent quarter/year (returns a matrix)
test <- sapply(nm, FUN = function(x) {grepl(x, colnames(pred)[-c(1:2)])})
## replace non-aligned quarter-years with zeroes (imposes that mice wont use those variables to predict one another)
pred[-c(1:2), -c(1:2)][test] <- 0 # first two rows are not year-quarter specific so skip
...
df.imp <- mice(df.pre_imp_wide, pred = pred, m = 10, maxit = 20, seed = 1)
# Time difference of 2.899244 secs

###

df.imp <- complete(df.imp, action="long", include = TRUE)
df.imp$ebit_per_at <- with(df.imp, as.numeric(df.imp$ebit / df.imp$at))
df.imp.mids<-as.mids(df.imp)

form <- as.formula("CLI ~ op_expo_ew_y + rg_expo_ew_y + ph_expo_ew_y + ebit + ebit_per_at + us_dummy + total_lobby")
fitimp <- with(df.imp.mids, form)

summary(pool(fitimp)) # pool results across calls
