...
summary(duplicates$NA_col)
## we get back 782 rows from this exercise. Still have 1520 rows that we need to 
## deal with 

## for these 782 rows, duplicate_flag = 1

# duplicates_out <- duplicates[, c("bvdid", "isin", "cc_expo_ew_2016",
...
#write.csv(duplicates_out, "xx_other/exposure_orbis_bvdidORgvkey_duplicated.csv", row.names=F)
...
## dont need to account for duplicate client_uuids anymore because we do this above
...
View(exposure_orbis_lobby_wide[check, c("bvdid", "isin", "cc_expo_ew_2016", "gvkey", "hqcountrycode", "conm", "total_assets_usd_2016", "client_uuid", "client_name")])
...
out <- exposure_orbis_lobby_wide[check, c("bvdid", "isin", "cc_expo_ew_2016", "gvkey", "hqcountrycode", "conm", "total_assets_usd_2016", "client_uuid", "client_name")]

write.csv(out, "xx_other/exposure_orbis_clientuuid_duplicated_additional.csv", row.names = F)
